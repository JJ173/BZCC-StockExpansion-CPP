...
const char *DLLBase::GetNextRandomVehicleODF(int Team)
{
	_ASSERTE(FALSE);
	return "player";
}
...
PreSnipeReturnCodes DLLBase::PreSnipe(const int curWorld, Handle shooterHandle, Handle victimHandle, int ordnanceTeam, const char* pOrdnanceODF)
{
...
void DLLBase::PreOrdnanceHit(Handle shooterHandle, Handle victimHandle, int ordnanceTeam, const char* pOrdnanceODF)
{
...
PreSnipeReturnCodes DLLAPI DLLBasePreSnipeCallback(const int curWorld, Handle shooterHandle, Handle victimHandle, int ordnanceTeam, const char* pOrdnanceODF)
{
	if(Mission)
	{
...
// function call via the DLLBase class
void DLLAPI DLLBasePreOrdnanceHitCallback(Handle shooterHandle, Handle victimHandle, int ordnanceTeam, const char* pOrdnanceODF)
{
	if(Mission)
	{
...
static void DLLAPI InitialSetup(void)
{
	if (Mission == NULL)
...
static bool DLLAPI Save(bool misnSave)
{
	return Mission->Save(misnSave);
}

static bool DLLAPI Load(bool misnSave)
{
	if (Mission == NULL)
		Mission = BuildMission();
...
static bool DLLAPI PostLoad(bool misnSave)
{
	return Mission->PostLoad(misnSave);
}

static void DLLAPI AddObject(Handle h)
{
	Mission->AddObject(h);
}

static void DLLAPI DeleteObject(Handle h)
{
	Mission->DeleteObject(h);
}

static void DLLAPI Update(void)
{
	Mission->Execute();
}

static void DLLAPI PostRun(void)
{
	Mission->PostRun();
	delete Mission;
}
...
static void DLLAPI DeletePlayer(DPID id)
{
	Mission->DeletePlayer(id);
}

static EjectKillRetCodes DLLAPI PlayerEjected(Handle DeadObjectHandle)
{
	return Mission->PlayerEjected(DeadObjectHandle);
}

static EjectKillRetCodes DLLAPI ObjectKilled(int DeadObjectHandle,int KillersHandle)
{
	return Mission->ObjectKilled(DeadObjectHandle, KillersHandle);
}

static EjectKillRetCodes DLLAPI ObjectSniped(int DeadObjectHandle,int KillersHandle)
{
...
static void DLLAPI SetWorld(int nextWorld)
{
	if (CurrentWorld >= 0)
...
static void DLLAPI ProcessCommand(unsigned long crc)
{
	Mission->ProcessCommand(crc);
}

static void DLLAPI SetRandomSeed(unsigned long seed)
{
